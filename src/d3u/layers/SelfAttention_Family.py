...
from src.d3u.utils.masking import TriangularCausalMask, ProbMask
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):
        ...
        Q_reduce = Q[
            torch.arange(B, device=K.device)[:, None, None],
            torch.arange(H, device=K.device)[None, :, None],
            M_top,
            :,
        ]
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        U_part = int(self.factor * np.ceil(np.log(L_K)).astype("int").item())
        u = int(self.factor * np.ceil(np.log(L_Q)).astype("int").item())
