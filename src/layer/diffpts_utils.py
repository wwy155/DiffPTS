...
def cal_forward_noise(betas_bar, gx, t):
...
# Reverse function -- sample y_{t-1} given y_t
def p_sample(model, x, x_mark, yt, y_0_hat, gx, fx, t, alphas, one_minus_alphas_bar_sqrt, alphas_cumprod,alphas_cumprod_sum, alpha_bar_prev, alphas_cumprod_sum_prev, betas_tiled_all, betas_bar_all, betas_tiled_m_1_all, betas_bar_m_1_all):
    """
    Reverse diffusion process sampling -- one time step.
    ...
    device = next(model.parameters()).device
    t = torch.tensor([t]).to(device)
    eps_theta, _ = model(x, x_mark, yt, y_0_hat, gx, t)
    eps_theta = eps_theta.to(device).detach()

    z =  torch.randn_like(yt)  # if t > 1 else torch.zeros_like(y)

    sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, yt)
    sqrt_one_minus_alpha_bar_t_m_1 = extract(one_minus_alphas_bar_sqrt, t - 1, yt)
    sqrt_alpha_bar_t = (1 - sqrt_one_minus_alpha_bar_t.square()).sqrt()
    sqrt_alpha_bar_t_m_1 = (1 - sqrt_one_minus_alpha_bar_t_m_1.square()).sqrt()
    betas_tiled_m_1 = extract(betas_tiled_m_1_all, t, yt)
    betas_bar_m_1 = extract(betas_bar_m_1_all, t, yt)
    betas_tiled = extract(betas_tiled_all, t, yt)
    betas_bar = extract(betas_bar_all, t, yt)
    ...
    y_0_hat =  1 / sqrt_alpha_bar_t  * ( yt - (1 - sqrt_alpha_bar_t) * fx - torch.sqrt(betas_tiled*gx)*eps_theta)

    mu_tilde = gamma_0 * y_0_hat + gamma_1 * yt + gamma_2 * fx
    y_t_m_1 = mu_tilde.to(device) +torch.sqrt(betas_tiled*gx)*z.to(device)

    # z =  torch.randn_like(y)  # if t > 1 else torch.zeros_like(y)



    # mu_tilde = (y_0_hat + (1 - sqrt_alpha_bar_t)*fx)/sqrt_alpha_bar_t

    # # y_0_reparam = 1 / sqrt_alpha_bar_t * (
    # #         y - (1 - sqrt_alpha_bar_t) * y_0_hat - eps_theta*torch.sqrt(noise))
    ...
    # log_p_yT_given_X = 0.5 * (2 * torch.pi * torch.log(gx) +  (y_0_hat - y).square() / gx)
    ...
    # y_t_m_1_hat = gamma_0 * y_0_reparam + gamma_1 * y + gamma_2 * y_T_mean
    # # posterior variance
    # y_t_m_1 = y_t_m_1_hat.to(device) + torch.sqrt(noise) *z.to(device)
    return y_t_m_1
