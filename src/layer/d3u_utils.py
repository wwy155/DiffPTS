...
# Forward functions
def q_sample(y, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t):
    """
...
# Reverse function -- sample y_{t-1} given y_t
def p_sample(model, x, x_mark, yt, enc_out, t, alphas, one_minus_alphas_bar_sqrt):
    device = next(model.parameters()).device
    t = torch.tensor([t]).to(device)
    eps_theta, _ = model( yt, t, enc_out)
    eps_theta = eps_theta.to(device).detach()

    z =  torch.randn_like(yt)  # if t > 1 else torch.zeros_like(y)      

    alpha_t = extract(alphas, t, yt)
    ...
    mu_tilde =    1/sqrt_alpha_bar_t * ( yt - (1 - alpha_t)/sqrt_one_minus_alpha_bar_t *eps_theta) #gamma_0 * y_0_hat + gamma_1 * yt + gamma_2 * fx
    sigma_tilde = sqrt_one_minus_alpha_bar_t_m_1/sqrt_one_minus_alpha_bar_t*(1 - alpha_t)
    y_t_m_1 = mu_tilde.to(device) +sigma_tilde*z.to(device)
...
def p_sample_t_1to0(model, x, x_mark, yt, enc_out, one_minus_alphas_bar_sqrt, betas_bar_all):
    device = next(model.parameters()).device
    t = torch.tensor([0]).to(device)  # corresponding to timestep 1 (i.e., t=1 in diffusion models)
    sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, yt)
    sqrt_alpha_bar_t = (1 - sqrt_one_minus_alpha_bar_t.square()).sqrt()
    ...
    y_0_reparam = 1 / sqrt_alpha_bar_t * (
            yt  - eps_theta * torch.sqrt(betas_bar))
    y_t_m_1 = y_0_reparam.to(device)
    return y_t_m_1
...
def p_sample_loop(model, x, x_mark, y_T, enc_out, n_steps, alphas, one_minus_alphas_bar_sqrt, betas_bar):
    ...
    for t in reversed(range(1, n_steps)):  # t from T to 2
        y_t = cur_y
        cur_y = p_sample(model, x, x_mark, y_t, enc_out, t, alphas, one_minus_alphas_bar_sqrt) # y_{t-1}
        y_p_seq.append(cur_y)
    assert len(y_p_seq) == n_steps
    y_0 = p_sample_t_1to0(model, x, x_mark, y_p_seq[-1], enc_out, one_minus_alphas_bar_sqrt,betas_bar)
    y_p_seq.append(y_0)
    return y_p_seq
