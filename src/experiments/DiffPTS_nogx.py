"""
DiffPTS ablation: **no gx training**
...
Additionally:
- gx is fixed to 1 (not learned / not predicted)
"""
...
class DiffPTSForecast(_BaseDiffPTSForecast):
    """
    Same CLI/Fire surface as `src/experiments/DiffPTS.py`, but with gx frozen.
    """
    model_type: str = "DiffPTS_nogx"

    def _init_model(self):
        super()._init_model()
        for p in self.cond_pred_model_g.parameters():
            p.requires_grad_(False)

    def _init_optimizer(self):
        # Only optimize diffusion + fx
    ...
    def _process_train_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        # Same as base, but gx is fixed to 1.
        ...
        gx = torch.ones_like(batch_y, device=self.device)
    ...
    def _process_val_batch(self, batch_x, batch_y, batch_x_mark, batch_y_mark, plot=True):
        ...
        gx = torch.ones_like(y_0_hat_batch, device=self.device)
